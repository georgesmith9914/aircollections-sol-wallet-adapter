...
function App() {
  ...
  async function sendTransaction() {
    try {
      ...
      addLog('Transaction ' + signature + ' confirmed in aircollections');
      window.alert("NFT is being transferred to your account. Please check your Solana wallet in a few minutes."); 
      //NFT needs to be preminzed and to be added to this user account 
      //Make backend call here to transfer NFT to this address
  ...
  return (
    <div className="App">
      <h1>AirCollections NFT Purchase</h1>
      ...
      {selectedWallet && selectedWallet.connected ? (
        <div>
          ...
          <button class="btn" onClick={sendTransaction}>Buy NFT</button>
          {/*<button onClick={signMessage}>Sign Message</button>*/}
          <button class="btndisconnect" onClick={() => selectedWallet.disconnect()}>Disconnect</button>
        </div>
      ) : (
        <div>
          <button class="btn" onClick={() => setSelectedWallet(urlWallet)}>Connect to Wallet</button>
